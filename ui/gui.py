...
class PageAllegroMonitored(QWidget):
    ...
    def mark_as_done(self, link):
        for element in self.gridLayout_scroll_area.children():
            if element.url_link == link:
                element.label_stat.setText("done")
                element.label_img_stat.setPixmap(QPixmap(os.path.join(path, "img/check.png")))
